defmodule CLI.ServerTest do
  ...
  setup do
    ...
    game_env = fn () ->
      Server.select({:play_game, 2, %{player_1: "player 1", player_2: "player 2", game_id: game_id}})
    end

    print_board = fn () ->
      Server.print_board(GameServer.get_game(game_id))
    end

    {:ok, %{main_menu: main_menu, new_two_player: new_two_player,
            new_one_player_easy: new_one_player_easy, game_env: game_env,
            print_board: print_board, game_id: game_id}}
  end
  ...
  describe "select({:play_game, 2, state})" do
    ...
    test "handles a draw", %{game_env: game_env, game_id: game_id} do
      fill_board(game_id)
      assert capture_io([input: "7\nq"], game_env) =~ "The game has ended in a draw! Try again..."
    end

    test "handles invalid input", %{game_env: game_env, game_id: game_id} do
  ...
  describe "print_board/1" do
    ...
    test "prints nearly full board", %{print_board: print_board, game_id: game_id} do
      fill_board(game_id)
